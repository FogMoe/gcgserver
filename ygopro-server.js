  ...
  ygopro.stoc_follow('GAME_MSG', true, async function(buffer, info, client, server, datas) {
    ...
    if (settings.modules.retry_handle.enabled) {
      if (msg_name === 'RETRY') {
        ...
        const msgPreview = client.last_game_msg ? client.last_game_msg.toString('hex') : '';
        const promptTitle = client.last_prompt_msg_title || 'UNKNOWN';
        const promptLen = client.last_prompt_msg ? client.last_prompt_msg.length : 0;
        const promptPreview = client.last_prompt_msg ? client.last_prompt_msg.toString('hex') : '';
  ...
  ygopro.stoc_follow('DUEL_START', false, async function(buffer, info, client, server, datas) {
    ...
    if (room.duel_stage === ygopro.constants.DUEL_STAGE.BEGIN) { //first start
      ...
      for (j = 0, len = ref.length; j < len; j++) {
        ...
        /** @type {Buffer} player.supplyRaw - 存储原始GCG Supply payload数据 */
